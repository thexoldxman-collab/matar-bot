...
api_connections = {}  # تخزين اتصالات API المؤقتة
...
# 4. دوال النسخ الفوري
# ==========================================
def create_copy_text(text):
    """إنشاء نص قابل للنسخ مع رسالة توجيهية"""
    return f"🔗 **لنسخ النص، اضغط عليه ثم اختر Copy**\n\n`{text}`"

def send_copyable_message(chat_id, text, caption=""):
    """إرسال رسالة تحتوي على نص قابل للنسخ"""
    message_text = f"{caption}\n\n{create_copy_text(text)}" if caption else create_copy_text(text)
    bot.send_message(chat_id, message_text, parse_mode="Markdown")

def handle_number_click(call, number_text, number_type):
    """معالجة الضغط على أي رقم لنسخه فوراً"""
    bot.answer_callback_query(call.id, "🔗 الرقم جاهز للنسخ", show_alert=False)
    send_copyable_message(call.message.chat.id, number_text, f"📋 {number_type}:")
...
def verify_syriatel_number(number):
    ...
    # التحقق من أن الرقم مكون من أرقام فقط
    if not number.isdigit():
        return False, "رقم غير صالح: يجب أن يحتوي على أرقام فقط"

    # التحقق من الطول (مثال: 12 خانة)
...
def verify_sham_address(address):
    ...
    # يمكن إضافة تحقق أكثر تقدماً حسب متطلبات شام كاش
    return True, "عنوان صحيح"

def test_api_connection(api_name, api_key, api_secret, endpoint):
    """اختبار اتصال API"""
    # هذا مجرد مثال - يجب تعديله حسب API الحقيقي
    try:
...
def format_ichancy_info(uid):
    ...
    # إنشاء النص مع أرقام قابلة للنسخ (سيتم إرسالها كرسائل منفصلة)
    text = (
        f"❤️ Ichancy ❤️\n\n"
        f"ℹ️ معلومات الحساب:\n"
        f"━━━━━━━━━━━━━━━━\n"
    ...
    # إنشاء أزرار للنسخ الفوري
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("📋 اسم المستخدم", callback_data=f"copy_name_{acc_name}"),
        types.InlineKeyboardButton("📋 كلمة السر", callback_data=f"copy_pass_{acc_password}"),
        types.InlineKeyboardButton("📋 المعرف", callback_data=f"copy_id_{uid}"),
        types.InlineKeyboardButton("📋 الرصيد", callback_data=f"copy_balance_{balance}")
    )
...
def get_admin_main_keyboard(is_owner=False):
    ...
    if is_owner:
        ...
        markup.add('🔗 ربط الكاشيرة')  # زر جديد للربط
    else:
...
# ==========================================
# 9. الراوتر الرئيسي للرسائل (مع منع التعليق)
# ==========================================
...
def main_router(m):
    ...
    # إعادة تعيين الحالة عند كل رسالة جديدة لمنع التعليق
    reset_user_state(uid)
    ...
    # البحث في الأزرار الديناميكية أولاً (نظام متقدم)
    ...
    if button_info:
        ...
        conn.commit()
        return
    # إذا لم يكن زرًا ديناميكيًا، نتحقق من الأزرار العادية (للحفاظ على التوافق مع الكود القديم)
...
def process_user_info(message):
    ...
    try:
        ...
        bot.send_message(uid, "❌ ID خطأ")
# ==========================================
# 11.5 دوال معالجة الإدخالات المتقدمة (جديدة)
# ==========================================
def process_add_button_advanced_step1(message):
    """الخطوة 1: إضافة زر متقدم - اسم الزر"""
    uid = message.from_user.id
    if uid != ADMIN_ID:
        return
    
    button_name = message.text
    msg = bot.send_message(uid, f"📝 أدخل الرسالة التي ستظهر عند الضغط على الزر (أو 'لا' لتخطي):")
...
def process_add_button_advanced_step2(message, button_name):
    """الخطوة 2: إضافة زر متقدم - الرسالة"""
    uid = message.from_user.id
    if uid != ADMIN_ID:
        return
    ...
    msg = bot.send_message(uid, f"🖼️ أرسل ID الصورة (أو 'لا' لتخطي):")
...
def process_add_button_advanced_step3(message, button_name, message_text):
    """الخطوة 3: إضافة زر متقدم - الصورة"""
    uid = message.from_user.id
    if uid != ADMIN_ID:
        return
    ...
    msg = bot.send_message(uid, f"🎯 أدخل الإجراء (مثل: show_balance, show_ichancy_menu, show_submenu أو 'لا' لتخطي):")
...
def process_add_button_advanced_step4(message, button_name, message_text, photo_id):
    """الخطوة 4: إضافة زر متقدم - الإجراء"""
    uid = message.from_user.id
    if uid != ADMIN_ID:
        return
    ...
    msg = bot.send_message(uid, f"📂 الزر الرئيسي (مثل: main أو اسم زر آخر للقوائم الفرعية):")
...
def process_add_button_advanced_step5(message, button_name, message_text, photo_id, action):
    """الخطوة 5: إضافة زر متقدم - المستوى"""
    uid = message.from_user.id
    if uid != ADMIN_ID:
        return
    ...
    msg = bot.send_message(uid, f"📊 المستوى (1 للرئيسي، 2 للفرعي):")
...
def process_create_submenu_step2(message, parent_button):
    ...
    # إضافة الزر الفرعي بالمستوى 2 والأب هو parent_button
...
# ==========================================
# 19. عمليات الشحن والسحب الخارجي
# ==========================================
...
def handle_charge_methods(call):
    ...
    if method == 'syria':
        ...
        # إرسال الأرقام كرسالة قابلة للنسخ
        bot.send_message(call.message.chat.id, "📋 اضغط على الرقم لنسخه:")
        send_copyable_message(call.message.chat.id, numbers, "أرقام سيرياتل كاش")
        msg = bot.send_message(call.message.chat.id, text)
        ...
        min_amount = get_db_setting('min_charge')
        text = f"💳 شام كاش\n\n📱 العنوان: {address}\n⚠️ الحد الأدنى: {min_amount}\n\n📝 أرسل رقم العملية:"
        bot.send_message(call.message.chat.id, "📋 اضغط على العنوان لنسخه:")
        send_copyable_message(call.message.chat.id, address, "عنوان شام كاش")
        msg = bot.send_message(call.message.chat.id, text)
...
def process_charge_amount(message, method, receipt):
    ...
    try:
        ...
        if auto_verify:
            # محاكاة تحقق (يجب ربطه بـ API حقيقي)
            verified = True
            if verified:
...
def handle_copy(call):
    ...
    if len(parts) >= 3:
        copy_type = parts[1]
        text = parts[2]
        bot.answer_callback_query(call.id, f"📋 تم تجهيز {copy_type} للنسخ", show_alert=False)
        send_copyable_message(call.message.chat.id, text, f"📋 {copy_type}:")
