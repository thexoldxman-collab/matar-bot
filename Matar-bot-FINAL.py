# =============================================================================
# MATAR ULTIMATE TELEGRAM BOT - ENTERPRISE EDITION v5.0
# =============================================================================
...
# نهاية الرسالة الأولى - يتبعها رسالة ثانية
...
class ActionSystem:
    ...
    def show_ichancy_menu(uid, chat_id):
        ...
        # النصوص الزرقاء القابلة للنسخ (سيتم إضافتها عبر inline keyboard)
        if dark_mode:
            text = f"""
🌙 **❤️ Ichancy ❤️**
...
"""
        ...
        # أزرار النسخ المضمنة (كل زر ينسخ نصاً معيناً عند الضغط)
        markup_inline = types.InlineKeyboardMarkup(row_width=3)
        markup_inline.add(
            types.InlineKeyboardButton("📋 اسم المستخدم", callback_data=f"copy_{acc[0]}"),
            types.InlineKeyboardButton("📋 كلمة السر", callback_data=f"copy_{acc[1]}"),
    ...
    def process_charge_ichancy(message, bot_balance):
        ...
        try:
            ...
            # التحقق من وضع API
            api_mode = get_db_setting('ichancy_api_mode')
...
# نهاية الرسالة الثانية - يتبعها رسالة ثالثة
...
def handle_all_callbacks(call):
    ...
    # كول باكات تعديل الأزرار (تظهر في وضع الإدارة)
    if data.startswith('edit_name_'):
        button_text = data[10:]
        msg = bot.send_message(call.message.chat.id, f"✏️ **أدخل الاسم الجديد للزر** `{button_text}`**:**", parse_mode="Markdown")
        bot.register_next_step_handler(msg, process_edit_name_callback, button_text)
    ...
    if data.startswith('delete_') and not data.startswith('delete_button_'):
        button_text = data[7:]
        confirm_markup = types.InlineKeyboardMarkup()
        confirm_markup.add(
            types.InlineKeyboardButton("✅ نعم، احذف", callback_data=f"confirm_delete_{button_text}"),
    ...
    if data.startswith('confirm_delete_'):
        ...
        bot.edit_message_text(f"✅ **تم حذف الزر** `{button_text}`", 
                            call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
        return
    ...
    if data.startswith('toggle_') and not data.startswith('toggle_bot'):
        button_text = data[7:]
        ...
        # تحديث الرسالة
        btn_details = get_button_details(button_text)
        if btn_details:
            text = f"""
🔧 **معلومات الزر:** `{button_text}`

🆔 المسار: {btn_details[4] if len(btn_details) > 4 else '/'}
📂 الرئيسي: {btn_details[3]}
📊 المستوى: {btn_details[4] if len(btn_details) > 4 else 1}
🎯 الإجراء: {btn_details[0] or 'لا يوجد'}
📝 الرسالة: {btn_details[1] or 'لا يوجد'[:30]}...
🖼️ صورة: {'نعم' if btn_details[2] else 'لا'}
🔁 الحالة: {'🟢 مفعل' if new_status == 1 else '🔴 معطل'}
"""
    ...
    admin_callbacks = ['admin_', 'add_button', 'edit_button', 'reorder_buttons', 'delete_button',
                      ...
                      'toggle_auto_verify', 'api_status', 'admin_logs', 'admin_search_logs', 'admin_notifications',
                      'admin_gifts', 'admin_referrals', 'payment_status', 'advanced_user_search', 'restore_user',
                      'create_user', 'auto_backup_settings', 'edit_retry', 'search_by_date', 'search_by_admin',
    ...
    if data == 'admin_buttons':
        ...
        bot.edit_message_text("🔁 **أرسل اسم الزر الذي تريد تفعيل/تعطيل:**", 
                            call.message.chat.id, call.message.message_id)
    ...
    # ===== إعدادات الدفع =====
        ...
        commission = get_db_setting('withdraw_commission')
        syriatel_api = get_db_setting('syriatel_api_enabled')
        sham_api = get_db_setting('sham_api_enabled')
        
        text = f"""
💳 **إعدادات الدفع الحالية:**

📱 **سيرياتل كاش:** `{syriatel}`
   🔌 API: {'🟢 مفعل' if syriatel_api == '1' else '🔴 معطل'}

🏦 **شام كاش:** `{sham}`
   🔌 API: {'🟢 مفعل' if sham_api == '1' else '🔴 معطل'}
...
"""
        ...
        bot.edit_message_text("📱 **أرسل أرقام سيرياتل كاش الجديدة (مفصولة بفواصل):**", 
                            call.message.chat.id, call.message.message_id)
        ...
        bot.edit_message_text("💰 **أرسل الحدود بالترتيب (الحد الأدنى للشحن، الحد الأدنى للسحب، الحد الأقصى للسحب):**\nمثال: 100,25000,500000", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_update_limits)
    
    elif data == 'edit_commission':
        bot.edit_message_text("💸 **أرسل نسبة العمولة الجديدة (رقم فقط):**", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_update_commission)
    
    elif data == 'toggle_api':
        current_syriatel = get_db_setting('syriatel_api_enabled')
        current_sham = get_db_setting('sham_api_enabled')
        
        text = f"""
🔌 **إعدادات API الحالية:**

📱 سيرياتل كاش: {'🟢 مفعل' if current_syriatel == '1' else '🔴 معطل'}
🏦 شام كاش: {'🟢 مفعل' if current_sham == '1' else '🔴 معطل'}

اختر الخدمة لتفعيل/تعطيل:
"""
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton("📱 سيرياتل", callback_data="toggle_syriatel_api"),
            types.InlineKeyboardButton("🏦 شام", callback_data="toggle_sham_api"),
            types.InlineKeyboardButton("🔙 رجوع", callback_data="back_to_payment")
        )
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                            reply_markup=markup, parse_mode="Markdown")
    
    elif data == 'toggle_syriatel_api':
        current = get_db_setting('syriatel_api_enabled')
        new = '0' if current == '1' else '1'
        update_db_setting('syriatel_api_enabled', new, uid)
        bot.answer_callback_query(call.id, f"✅ تم {'تفعيل' if new == '1' else 'تعطيل'} API سيرياتل")
        
        # العودة لقائمة الدفع
        bot.edit_message_text("💳 **إعدادات الدفع**", call.message.chat.id, call.message.message_id,
                            reply_markup=get_payment_settings_keyboard())
    
    elif data == 'toggle_sham_api':
        current = get_db_setting('sham_api_enabled')
        new = '0' if current == '1' else '1'
        update_db_setting('sham_api_enabled', new, uid)
        bot.answer_callback_query(call.id, f"✅ تم {'تفعيل' if new == '1' else 'تعطيل'} API شام")
...
"""
        ...
        bot.edit_message_text("📝 **أرسل معرف المستخدم (ID) أو اسمه للبحث:**", 
                            call.message.chat.id, call.message.message_id)
    ...
    # ===== الإعدادات العامة =====
        ...
        retry_attempts = get_db_setting('retry_attempts')
        retry_delay = get_db_setting('retry_delay')
        dark_mode = get_db_setting('enable_dark_mode')
        
        status_text = "🟢 نشط" if status == 'active' else "🔴 معطل"
        dark_text = "🟢 مفعل" if dark_mode == '1' else "🔴 معطل"
...
"""
        ...
        bot.edit_message_text("📝 **أرسل رسالة الترحيب الجديدة:**", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_edit_welcome)
    
    elif data == 'edit_terms':
        bot.edit_message_text("📜 **أرسل نص الشروط الجديد:**", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_edit_terms)
    
    elif data == 'edit_referral':
        bot.edit_message_text("👥 **أرسل نسبة الإحالات الجديدة (رقم فقط):**", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_edit_referral)
    
    elif data == 'edit_retry':
        bot.edit_message_text("🔄 **أرسل إعدادات إعادة المحاولة (عدد المحاولات,المدة بالدقائق):**\nمثال: 3,3", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_edit_retry)
    
    elif data == 'toggle_dark_mode_global':
        current = get_db_setting('enable_dark_mode')
        new = '0' if current == '1' else '1'
        update_db_setting('enable_dark_mode', new, uid)
        
        status = "🟢 مفعل" if new == '1' else "🔴 معطل"
        bot.answer_callback_query(call.id, f"✅ تم تغيير الوضع الليلي إلى: {status}", show_alert=True)
        
        # العودة للإعدادات
        bot.edit_message_text("⚙️ **الإعدادات العامة**", call.message.chat.id, call.message.message_id,
    ...
    elif data == 'broadcast_moderators':
        bot.edit_message_text("👥 **أرسل الرسالة التي تريد إرسالها للمشرفين فقط:**", 
                            call.message.chat.id, call.message.message_id)
    ...
    # ===== ربط الكاشيرة =====
        ...
        syriatel = get_db_setting('syriatel_api_enabled')
        sham = get_db_setting('sham_api_enabled')
        auto = get_db_setting('auto_verify_charges')
        
        text = f"""
📊 **حالة اتصالات API:**

📱 سيرياتل كاش: {'🟢 مفعل' if syriatel == '1' else '🔴 معطل'}
🏦 شام كاش: {'🟢 مفعل' if sham == '1' else '🔴 معطل'}
⚡ التحقق التلقائي: {'🟢 مفعل' if auto == '1' else '🔴 معطل'}
...
"""
    ...
    elif data == 'rename_moderator':
        bot.edit_message_text("✏️ **أرسل معرف المستخدم (ID) لإعادة تسميته:**", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_rename_moderator_step1)
    
    elif data == 'moderator_permissions':
        bot.edit_message_text("🔒 **أرسل معرف المستخدم (ID) لتعديل صلاحياته:**", 
                            call.message.chat.id, call.message.message_id)
    ...
    # ===== النسخ الاحتياطي =====
        ...
        log_admin_action(uid, "نسخ احتياطي", f"حفظ نسخة {backup_name}")
    
    elif data == 'export_db':
        bot.answer_callback_query(call.id, "📤 جاري تصدير قاعدة البيانات...")
        time.sleep(2)
        bot.send_message(call.message.chat.id, "✅ **تم تصدير قاعدة البيانات بنجاح**\n📁 الملف: matar_ultimate_export.db", parse_mode="Markdown")
    
    elif data == 'list_backups':
        cursor.execute("SELECT backup_name, created_at, created_by FROM backups ORDER BY created_at DESC LIMIT 10")
        backups = cursor.fetchall()
        
        if not backups:
            bot.send_message(call.message.chat.id, "📭 **لا توجد نسخ احتياطية**", parse_mode="Markdown")
            return
        
        text = "📋 **آخر 10 نسخ احتياطية:**\n\n"
        for b in backups:
            text += f"🔹 `{b[0]}`\n   📅 {b[1]} | 👤 {b[2]}\n\n"
    ...
    # ===== سجل الإجراءات =====
        ...
        bot.edit_message_text("🔍 **بحث متقدم في سجل الإجراءات**\n\nاختر معيار البحث:", 
                            call.message.chat.id, call.message.message_id,
                            reply_markup=get_search_keyboard(), parse_mode="Markdown")
    
    elif data == 'search_by_date':
        bot.edit_message_text("📅 **أدخل تاريخ البدء وتاريخ الانتهاء (YYYY-MM-DD,YYYY-MM-DD):**\nمثال: 2024-02-01,2024-02-20", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_search_by_date)
    
    elif data == 'search_by_admin':
        bot.edit_message_text("👤 **أدخل اسم المشرف أو ID:**", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_search_by_admin)
    
    elif data == 'search_by_type':
        bot.edit_message_text("🔍 **أدخل نوع الإجراء (مثال: حظر، شحن، قبول):**", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_search_by_type)
    
    elif data == 'search_by_amount':
        bot.edit_message_text("💰 **أدخل نطاق المبلغ (min,max):**\nمثال: 1000,50000", 
                            call.message.chat.id, call.message.message_id)
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_search_by_amount)
    
    elif data == 'search_by_user':
        bot.edit_message_text("👥 **أدخل ID المستخدم المستهدف:**", 
                            call.message.chat.id, call.message.message_id)
    ...
    # ===== إعدادات الإشعارات =====
        ...
        cursor.execute("UPDATE notification_settings SET notify_on_charge=1, notify_on_withdraw=1, notify_on_ticket=1, notify_on_new_user=1, notify_on_expired_gift=1 WHERE user_id=?", (ADMIN_ID,))
        conn.commit()
        bot.answer_callback_query(call.id, "✅ تم تفعيل جميع الإشعارات")
        ActionSystem.notification_settings(uid, call.message.chat.id)
    
    elif data == 'edit_charge_threshold':
        bot.edit_message_text("💰 **أدخل الحد الجديد للشحن الكبير:**", 
                            call.message.chat.id, call.message.message_id)
...
اختر نوع الهدية التي تريد إنشاءها:
"""
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id,
                            reply_markup=get_gift_type_keyboard(), parse_mode="Markdown")
    
    elif data == 'gift_individual':
        msg = bot.send_message(call.message.chat.id, "💰 **أدخل قيمة الهدية:**")
        bot.register_next_step_handler(msg, process_individual_gift, call.message.chat.id)
        bot.answer_callback_query(call.id)
    
    elif data == 'gift_group':
        msg = bot.send_message(call.message.chat.id, "👥 **أدخل عدد المستخدمين:**")
        bot.register_next_step_handler(msg, process_group_gift_count, call.message.chat.id)
        bot.answer_callback_query(call.id)
    
    # ===== نظام الإحالات =====
    elif data == 'admin_referrals':
        ActionSystem.show_referral_stats(uid, call.message.chat.id)
    ...
    elif data == 'back_to_button_management':
        bot.edit_message_text("🔧 **إدارة الأزرار**", call.message.chat.id, call.message.message_id,
                            reply_markup=get_buttons_management_keyboard())

# نهاية الرسالة الرابعة - يتبعها رسالة خامسة وأخيرة
...
def process_edit_name_callback(message, button_text):
    """معالجة تعديل اسم زر من الكول باك"""
    uid = message.from_user.id
    new_text = message.text
...
@app.route('/')
def home():
    return "🚀 Matar Ultimate Bot is Running! - Enterprise Edition v5.0"
...
if __name__ == "__main__":
    ...
    # رسالة بدء التشغيل
    logger.info("=" * 60)
    logger.info("🚀 MATAR ULTIMATE BOT - ENTERPRISE EDITION v5.0")
    logger.info("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("🚀 MATAR ULTIMATE BOT - ENTERPRISE EDITION v5.0")
    print("=" * 60)
...
# =============================================================================
# نهاية الكود - Matar Ultimate Bot v5.0
# =============================================================================