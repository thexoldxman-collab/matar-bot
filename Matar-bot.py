...
# ==========================================
# 2. نظام قاعدة البيانات المتكامل (محدث)
# ==========================================
def setup_database():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS users(
        ...
        first_name TEXT,
        username TEXT,
        custom_name TEXT,  -- للاسم المعاد تسميته (يظهر للمشرفين فقط)
        ref_code TEXT UNIQUE,  -- كود الإحالة الخاص
        referred_by INTEGER,  -- مين دعاه
        referral_count INTEGER DEFAULT 0,  -- عدد الإحالات
        current_earnings REAL DEFAULT 0,  -- أرباح الدورة الحالية
        total_earnings REAL DEFAULT 0)  -- إجمالي الأرباح
    """)
    ...
    # جدول التذاكر (تواصل مع الدعم) - محدث
    cursor.execute("""CREATE TABLE IF NOT EXISTS tickets(
        ...
        created_at TEXT,
        last_reply_by INTEGER,  -- آخر من رد (مالك أو مشرف)
        last_reply_at TEXT,
        replied_count INTEGER DEFAULT 0)""")
    
    # جدول محادثات التذاكر
    cursor.execute("""CREATE TABLE IF NOT EXISTS ticket_conversations(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ticket_id INTEGER,
        sender_id INTEGER,
        sender_type TEXT,  -- 'user', 'admin', 'moderator'
        message TEXT,
        file_id TEXT,
        sent_at TEXT)""")
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS moderators(
        user_id INTEGER PRIMARY KEY,
        custom_name TEXT,  -- الاسم المعاد تسميته
        added_by INTEGER,
        added_at TEXT,
        can_reply INTEGER DEFAULT 1,
        can_change_codes INTEGER DEFAULT 0,
        can_view_transactions INTEGER DEFAULT 0,
    ...
    # جدول سجل الإحالات (جديد)
    cursor.execute("""CREATE TABLE IF NOT EXISTS referrals_log(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        referrer_id INTEGER,  -- المحيل
        referred_id INTEGER,  -- المدعو
        joined_at TEXT,
        UNIQUE(referred_id))""")
    
    # جدول أرباح الإحالات (جديد)
    ...
    default_settings = [
        ...
        ('referral_percentage', '10'),  # نسبة الإحالة
        ('next_referral_payout', ''),  # موعد التوزيع القادم
    ...
    # بدء دورة إحالات أولى إذا لم توجد
...
# ==========================================
# 3. الوظائف المساعدة الأساسية (محدثة)
# ==========================================
...
def get_moderator_name(user_id):
    """الحصول على اسم المشرف (المعاد تسميته أو المعرف)"""
...
def get_user_custom_name(user_id):
    """الحصول على الاسم المعاد تسميته للمستخدم"""
...
def process_referral_charge(user_id, charge_amount, transaction_id):
    """معالجة شحن شخص مدعو وإضافة أرباح للمحيل"""
...
def check_and_create_ref_code(user_id):
    """التأكد من وجود كود إحالة للمستخدم وإنشاء واحد إذا لم يوجد"""
...
def get_user_by_ref_code(ref_code):
    """الحصول على معرف المستخدم من كود الإحالة"""
...
def has_completed_welcome(user_id):
    """التحقق مما إذا كان المستخدم استقبل رسالة الترحيب"""
    cursor.execute("SELECT created_at FROM users WHERE user_id=?", (user_id,))
    result = cursor.fetchone()
    # إذا كان تاريخ الإنشاء قريب جداً (أقل من دقيقة) فهو مستخدم جديد
    if result and result[0]:
        try:
            created = datetime.strptime(result[0], "%d-%m-%Y %H:%M")
            now = datetime.now()
            diff = now - created
            return diff.total_seconds() > 60  # أكثر من دقيقة = مستخدم قديم
        except:
            return False
    return False

# ==========================================
# 4. بناء القوائم (Keyboards) - تصميم بوت الامبراطور
# ==========================================
def get_main_keyboard(uid):
    """القائمة الرئيسية (نسخة عن بوت الامبراطور مع إضافاتنا)"""
    markup = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    
    # أول زر منفرد: Ichancy
...
def get_ichancy_account_keyboard():
    """لوحة Ichancy بعد إنشاء الحساب (الأزرار تحت)"""
    markup = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    markup.add(
...
def format_ichancy_info(acc_name, acc_password, user_id, created_at):
    """تنسيق معلومات Ichancy (مثل بوت الامبراطور)"""
    return (
        f"👤 الحساب: {acc_name}\n"
...
def get_charge_methods_keyboard():
    """طرق الشحن في البوت (نسخة عن بوت الامبراطور)"""
...
def get_withdraw_methods_keyboard():
    """طرق السحب من البوت (نسخة عن بوت الامبراطور)"""
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
...
def get_admin_main_keyboard(is_owner=False):
    """لوحة الإدارة الرئيسية (للمالك أو المشرف)"""
    markup = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
...
def get_moderator_management_keyboard():
    """لوحة إدارة المشرفين (للمالك فقط)"""
    markup = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
...
def handle_start(message):
    ...
    if ref_code and ref_code.startswith('ref_'):
        ...
        if referrer_id and referrer_id != uid:
            # التحقق من أن المدعو لم يسجل من قبل
    ...
    # التأكد من إنشاء كود إحالة للمستخدم
    ...
    # التحقق مما إذا كانت هذه أول مرة (نرسل رسالة الترحيب مرة واحدة)
...
# ==========================================
# 6. الراوتر الرئيسي للرسائل (محدث)
# ==========================================
...
def main_router(m):
    ...
    # تصفير الخطوات لأي زر (لضمان عدم التعليق)
    reset_user_steps(uid)
    ...
    if text == '⚽ Ichancy ⚽':
        ...
        if not user_data or not user_data[0] or user_data[3] == 1:
            ...
            # يوجد حساب - عرض معلومات Ichancy (مثل بوت الامبراطور)
...
# ==========================================
# 7. نظام الإحالات (للمستخدم العادي)
# ==========================================
def show_referral_info(uid):
    """عرض معلومات الإحالات للمستخدم"""
    ...
    # الحصول على موعد التوزيع القادم
...
def process_gift_user_id(message):
    ...
    try:
        ...
        # التحقق من وجود المستهدف في قاعدة البيانات
...
def process_charge_amount(message, method, receipt):
    ...
    try:
        ...
        # تسجيل المعاملة ومعالجة الإحالة
        ...
        # إشعار المشرفين (باستثناء المالك)
...
# ==========================================
# 13. نظام التواصل مع الدعم (تذاكر متطورة)
# ==========================================
...
def handle_reply_ticket(call):
    ...
    # التحقق مما إذا كان التذكرة مغلقة أو تم الرد عليها
...
def process_add_moderator(message):
    ...
    try:
        ...
        # إرسال لوحة الإدارة للمشرف الجديد
...
# ==========================================
# 16. الشروط والاحكام (بدون اسم مبرمج)
# ==========================================
